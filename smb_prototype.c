...
int main(void) {
	...
	char * PACKET_1;
	memset(PACKET_1, "", 74);
	...
	printf("length PACKET_1 : %d", strlen(PACKET_1));
	...
	Sleep(3000);
	result = send(SMB_CONNECTION, PACKET_1, (int) strlen(PACKET_1), 0);
	printf("sent %d bytes\n", result);
	Sleep(3000);
	result = shutdown(SMB_CONNECTION, SD_SEND);
	do {
		result = recv(SMB_CONNECTION, PACKET_RECV, 512, 0);
		if(result > 0) {
